	...
	public function add() {
		if ($this->request->is('post')) {
			if ($ticket = $this->Ticket->save($this->request->data)) {
				if (!empty($ticket)) {
					// The ID of the newly created ticket has been set
					// as $this->Ticket->id.
					$this->request->data['Action']['ticket_id'] = $this->Ticket->id;
					$this->request->data['Action']['user_id'] = $this->Session->read('Auth.User.id');
					$this->request->data['Action']['project_id'] = $this->request->data['Ticket']['project_id'];
					$this->request->data['Action']['status'] = "new";
					$this->request->data['Action']['action'] = "created";

					$this->Ticket->Action->create();
					$this->Ticket->Action->save($this->request->data);
				}
